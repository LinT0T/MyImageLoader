...
public class MyImageLoader {
    ...
    public void load() {
        ...
        if (bit != null && use_cache) {
            ...
            bit = getBitmapFromFile(src.substring(src.length() - 9, src.length() - 5));
            if (bit != null && use_cache) {
                Message message = new Message();
                message.what = 200;
                message.obj = bit;
                ...
                myThreadPool.setMode(MyThreadPool.USE_FIXEDTHREADPOOL).setMaxThread(maxThread).setTaskNumber(taskNumber).setRunnable(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            String path = src.substring(src.length() - 9, src.length() - 5);
    ...
    public void loads(String s) {
        ...
        if (bit != null) {
            ...
            bit = getBitmapFromFile(st.substring(st.length() - 9, st.length() - 5));
            if (bit != null) {
                Message message = new Message();
                message.what = 200;
                message.obj = bit;
                ...
                myThreadPool.setMode(MyThreadPool.USE_FIXEDTHREADPOOL).setTaskNumber(1).setMaxThread(maxThread).setRunnable(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            String path = st.substring(st.length() - 9, st.length() - 5);
                            writeToLoce(path, bitmap);
                            handler2.sendMessage(message);
                        } catch (IOException e) {
                            e.printStackTrace();
